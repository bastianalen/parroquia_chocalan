...
/*generarCadenaAleatoria() genera una cadena aleatoria
 compuesta por números y letras (mayúsculas y minúsculas) 
 para ser utilizada posteriormente en la generación de nombres únicos de archivos.
 */
...
/*Se verifica la acción solicitada a través del parámetro GET action.
...
/*doInsert() se ejecuta cuando se envía el formulario para agregar una nueva persona fallecida ($_POST['save']).
Se procesan y validan los datos del formulario.
Se crea un objeto Persona, se asignan los valores y se llama al método create() para insertar los datos en la base de datos. */
function doInsert()
{
	global $mydb;
	if (isset($_POST['save'])) {
...
/*doEdit() maneja la actualización de un registro existente de persona fallecida ($_POST['save']).
Se procesan y validan los datos del formulario de edición.
Se crea un objeto Persona, se asignan los nuevos valores y se llama al método update() para actualizar los datos en la base de datos. */
function doEdit()
{
...
/*doDelete() se encarga de eliminar uno o varios registros de personas fallecidas seleccionados ($_POST['selector']).
Se validan los datos y se ejecuta el método delete() de la clase Persona para eliminar los registros correspondientes en la base de datos. */
function doDelete()
{
	global $mydb;
...
/*actualizarArchivo() gestiona la actualización de archivos adjuntos en los formularios.
Verifica si se cargó un archivo nuevo, genera un nombre único usando generarCadenaAleatoria(), y lo mueve a una carpeta de destino.
Además, puede mover archivos antiguos a una carpeta de archivos antiguos si se especifica $nuevoNombreCarpeta. */
...
/*validarDatos() valida campos críticos como el Rut, el nombre del fallecido, 
el sector y el tipo de tumba antes de proceder con la inserción o actualización. */
