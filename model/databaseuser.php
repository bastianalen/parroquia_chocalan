...
require_once(LIB_PATH.DS."conexionuser.php");
...
class DatabaseUser {
    ...
    public function open_connection() {
        $this->conn = mysqli_connect(server_user, user_user, pass_user,database_name_user,3306);
        if (!$this->conn) {
            throw new mysqli_sql_exception("Problem in database connection! Contact administrator!");
        } else {
            $db_select = mysqli_select_db($this->conn, database_name_user);
            if (!$db_select) {
